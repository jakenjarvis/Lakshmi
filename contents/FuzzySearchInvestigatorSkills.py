...
class SearchKeyword():
    ...
    def set_initialize(self, link_name, main_name, sub_name):
...
class FuzzySearchInvestigatorSkills():
    ...
    def __create_keywords(self):
        # SAN値
        keywords = SearchKeyword().set_initialize("sanity_points", "SAN", "")
        ...
        for key in self.investigator.characteristics.keys():
            if not key in exclusion_keys:
                ...
                keywords = SearchKeyword().set_initialize(key, ability_set.ability_name, "")
                self.__registration_of_similar_words(keywords)

        # 戦闘技能
        for key in self.investigator.combat_skills.keys():
            skill_set = self.investigator.combat_skills[key]
            keywords = SearchKeyword().set_initialize(key, skill_set.skill_name, skill_set.skill_subname)
            self.__registration_of_similar_words(keywords)
    ...
    def search(self, text: str) -> List[SearchKeyword]:
        ...
        df = pd.DataFrame(columns=["link_name", "main_name", "sub_name", "max_main_distance", "max_sub_distance", 'sum_distance'])
